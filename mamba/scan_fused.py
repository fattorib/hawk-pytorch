...
def parallel_scan_fwd_kernel(
    ...
    for chan_idx in range(N):
        ...
        deltaA_ch = tl.exp2(ln_2 * A_ch * delta)  # [L]
...
# fmt: on
    ...
    for chan_idx in range(N):
        ...
        # run forward scan to create hidden activation
        ...
        deltaA_ch_shifted = tl.exp2(ln_2 * A_ch * delta_shifted)  # [L]
        deltaA_ch_shifted = tl.where(tl.arange(0, BLOCKSIZE_L) < (BLOCKSIZE_L - 1), deltaA_ch_shifted, 0.0)

        # run reverse scan to create hidden grad
    ...
    ddelta = ddelta * softplus_bwd(ln_2*delta)
...
class MambaRecurrence(Function):
    ...
    def forward(ctx, A, delta, B, C, x) -> torch.Tensor:
        ...
        o = parallel_scan_fwd(A, delta, B, C, x)

...
def sequential_scan_fwd_kernel(
    ...
    alpha_ptr += bs_pid * bs_stride + BLOCKSIZE*channel_pid
    beta_ptr += bs_pid * bs_stride + BLOCKSIZE*channel_pid
    hidden_ptr += bs_pid * bs_stride + BLOCKSIZE*channel_pid
...
def sequential_scan_bwd_kernel(
    ...
    alpha_saved_ptr += (bs_pid * bs_stride) + ((num_context)*sq_stride) + BLOCKSIZE*channel_pid
    h_saved_ptr += (bs_pid * bs_stride) + ((num_context -2)*sq_stride) + BLOCKSIZE*channel_pid
    d_out_ptr += (bs_pid * bs_stride) + ((num_context-1)*sq_stride) + BLOCKSIZE*channel_pid

    d_alpha_ptr += (bs_pid * bs_stride) + ((num_context-1)*sq_stride) + BLOCKSIZE*channel_pid
    d_beta_ptr += (bs_pid * bs_stride) + ((num_context-1)*sq_stride) + BLOCKSIZE*channel_pid
...
def sequential_scan_forward(
    ...
    BLOCKSIZE = 1024
    channels = d // BLOCKSIZE

    assert d % BLOCKSIZE == 0
